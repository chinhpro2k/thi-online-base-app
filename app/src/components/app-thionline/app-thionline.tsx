...
import { Button, Col, Drawer, Modal, message, Row, Spin, Table, Select, Card } from 'antd'
...
import './index.less'
import LamBai from '@/src/components/app-thionline/lambai'
...
export default class AppThiOnline extends React.Component<AppThiOnlineProps, AppThiOnlineState> {
  ...
  async componentDidMount() {
    console.log('user ccccaas',this.props.user)
  ...
  render(): JSX.Element {
    ...
    const renderLast = (value: unknown, record: InfoCaThi) => {
      if (record.daLam) {
        ...
        if (
          ...
          return (
            <React.Fragment>
              <Button type="primary" icon={<SearchOutlined />} onClick={() => this.handleVaoCaThi(record)}>
                Vào thi
              </Button>
            </React.Fragment>
          )
    ...
    const titleCard: ReactNode = (
      <div style={{ marginTop: '20px', marginBottom: '20px' }}>
        Chọn kì học:{' '}
        <Select defaultValue={''} style={{ width: 250 }} onChange={this.handleChangeCourse}>
          {this.state.dsKyHoc.map((value, i) => {
            return (
              <Option value={value.ma_ky_nam_hoc} key={i}>
                {value.nam_hoc_id[1]} - Kỳ {value.ten_ky_nam_hoc} -{value.hinh_thuc_dao_tao_id[1]}
              </Option>
            )
          })}
        </Select>
        <Button style={{ marginLeft: '20px' }} type={'primary'} onClick={() => this.handleReload()}>
          Làm mới
        </Button>
      </div>
    )
    ...
    if (trangThaiThi?.dangThi) {
          ...
          <Row>
            <Col xs={24} md={12} lg={18}>
              <LamBai
                ...
                thoiGianKetThuc={trangThaiThi.thoiGianKetThuc ? trangThaiThi.thoiGianKetThuc : ''}
            ...
            <Col xs={24} md={12} lg={6}>
              {!!trangThaiThi?.deThi?.daLam ? (
                ...
                <div>
                  ...
                  {/*  navigate={(to) => this.handleNavigate(to)}*/}
                  {/*/>*/}
                  <Proctor
                    onReady={() => {
      ...
      ConditonalBlock = (
        <Row gutter={24} style={{ marginTop: '20px' }}>
            ...
            <Spin spinning={this.state.loading}>
              <Drawer
                ...
                footer={
                  ...
                  >
                    <Button onClick={() => this.handleCloseModal()} style={{ marginRight: 8 }}>
                      Đóng
                    </Button>
                  </div>
                }
              >
                <ChonLichThi dsLichThi={this.state.dsLichThi} onVaoThi={this.xacNhanVaoThi} />
              </Drawer>
          ...
          {/*<Col xs={24} md={12} lg={6}>*/}
          {/*  <UserInfo user={user} dispatch={this.props.dispatch} navigate={(to) => this.handleNavigate(to)} />*/}
          {/*</Col>*/}
        </Row>
      )
    ...
    return <div>
      <div>
        <AppHeader user={user} dispatch={this.props.dispatch} navigate={(to) => this.handleNavigate(to)}/>
      </div>
      {ConditonalBlock}
    </div>
  }
